...
2026-09-06 新增独立实例差分和 `scan storage` 的 Undo 证据追踪：已验证普通 `0x1D` 页的
used 区间、部分 Undo 公共头及两次 UPDATE 的前地址链。ROLL 页清零、复用和事务管理页的
变化说明仍不能由此得到完整提交集合。详见 [事务与 Undo 差分证据](transaction-undo-evidence.md)。
